...
> `git init` ကို project folder ထဲမှာပဲ run ပါ အပြင်မှာ (root folder) လိုမျိုးမှာ မ run သင့်ပါ။

----

# ပထမဆုံး git email နဲ့ password မထည့်ရသေးရင်

`git config --global credential.helper store` ကိုတစ်ချက် run ရပါမယ်

ပြီးလျှင် git pull သို့မဟုတ် git push နဲ့စမ်းလိုက်တာနဲ့ email နဲ့ password အလိုလိုတောင်းပါလိမ့်မယ်
